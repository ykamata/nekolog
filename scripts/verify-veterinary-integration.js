...
function checkFileContent(filePath, searchPattern, description) {
  try {
    ...
    if (
      ...
      (searchPattern instanceof RegExp && searchPattern.test(content))
    ) {
...
function verifyAuthIntegration() {
  ...
  const authChecks = [
    ...
    // 予約管理ページでの認証確認
    {
      ...
      check: () =>
        checkFileContent(
          'pages/veterinary-appointments.vue',
          "middleware: 'auth'",
          '予約管理ページの認証設定'
        ),
    },
    ...
    // APIエンドポイントでの認証確認
    {
      ...
      check: () =>
        checkFileContent(
          'server/api/veterinary-visits/index.get.ts',
          'auth-middleware',
          '通院記録API認証'
        ),
    },
...
function verifyCatDataIntegration() {
  ...
  const catChecks = [
    // 猫データ型定義の確認
    {
      description: '猫データ型定義',
      check: () =>
        checkFileExists('types/cat-meal.ts', '猫データ型定義') ||
        checkFileExists('types/index.d.ts', '猫データ型定義'),
    },
    ...
    // 通院記録フォームでの猫選択確認
    {
      ...
      check: () =>
        checkFileContent(
          'components/VeterinaryVisitForm.vue',
          'catId',
          '通院記録フォームでの猫選択'
        ),
    },
...
function verifyNavigationIntegration() {
  ...
  const navChecks = [
    // デフォルトレイアウトの確認
    {
      ...
      check: () =>
        checkFileExists('layouts/default.vue', 'デフォルトレイアウト'),
    },
...
function verifyDatabaseIntegration() {
  ...
  const dbChecks = [
    // Prismaスキーマの確認
    {
      ...
      check: () =>
        checkFileExists('prisma/schema.prisma', 'Prismaスキーマファイル'),
    },
    ...
    // マイグレーションファイルの確認
    {
      ...
      check: () =>
        checkFileExists(
          'prisma/migrations/20250729135209_add_veterinary_visit_management/migration.sql',
          'マイグレーションファイル'
        ),
    },
  ];
...
function verifyTypeIntegration() {
  ...
  const typeChecks = [
    // 通院記録型定義の確認
    {
      description: '通院記録型定義',
      check: () =>
        checkFileExists('types/veterinary-visit.ts', '通院記録型定義'),
    },
    ...
    // バリデーションスキーマの確認
    {
      ...
      check: () =>
        checkFileExists(
          'lib/validations/veterinary-visit.ts',
          'バリデーションスキーマ'
        ),
    },
...
function runIntegrationTests() {
  ...
  try {
    ...
    execSync(
      'pnpm exec eslint pages/veterinary-*.vue components/Veterinary*.vue server/api/veterinary-*',
      { stdio: 'pipe' }
    );
  ...
  try {
    ...
    execSync(
      'pnpm exec vitest run --reporter=basic tests/api/veterinary-*.test.ts tests/components/Veterinary*.test.ts',
      { stdio: 'pipe' }
    );
    logSuccess('単体テスト: 成功');
  } catch (error) {
...
function main() {
  log(
    `${COLORS.BOLD}${COLORS.BLUE}通院履歴管理機能 - 既存システム統合確認${COLORS.RESET}`
  );
  log(
    'このスクリプトは通院履歴管理機能と既存システムの統合状態を確認します。\n'
  );
