...
const querySchema = z.object({
  ...
  hasBloodTest: z.string().optional().transform(val => val === 'true'),
  limit: z
    .string()
    .transform(Number)
...
export default defineEventHandler(async (event) => {
  try {
    ...
    const { catId, hospitalId, doctorId, startDate, endDate, hasBloodTest, limit, offset }
    ...
    const cacheMaxAge = hasBloodTest !== undefined || startDate || endDate ? 30 : 300; // 30s for filtered, 5min for general
