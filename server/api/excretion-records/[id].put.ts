...
const paramsSchema = z.object({
  id: z.string().cuid('Invalid excretion record ID format'),
});
...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (updateData.type) finalUpdateData.type = updateData.type as 'URINE' | 'FECES';
    if (updateData.recordedAt) finalUpdateData.recordedAt = updateData.recordedAt;
