-- MySQL Initialization Script for Nekolog
-- This script is executed automatically when the MySQL container starts for the first time

-- Select the nekolog database (already created by docker-compose environment variables)
USE nekolog;

-- Grant all privileges to ykamata user (same as root)
GRANT ALL PRIVILEGES ON *.* TO 'ykamata'@'%' WITH GRANT OPTION;
FLUSH PRIVILEGES;
